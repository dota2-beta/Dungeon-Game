...
public class GameMapHex {
    ...
    private final List<SpawnPointInfo> playerSpawnPoints;
    private final List<SpawnPointInfo> monsterSpawnPoints;

    private int nextPlayerSpawnIndex = 0;

    @Builder
    public GameMapHex(Map<Hex, Tile> tiles, List<SpawnPointInfo> playerSpawnPoints, List<SpawnPointInfo> monsterSpawnPoints) {
    ...
    public Hex getAvailablePlayerSpawnPoint() {
        ...
        Hex spawnPoint = playerSpawnPoints.get(nextPlayerSpawnIndex).getPosition();
        nextPlayerSpawnIndex = (nextPlayerSpawnIndex + 1) % playerSpawnPoints.size();
        return spawnPoint;
    }
    /**
     * Возвращает список спавнпоинтов для монстров определенного типа
     * @return Список координат для спавна.
     */
    public List<Hex> getAvailableMonsterSpawnPoints(String templateId) {
        if (monsterSpawnPoints.isEmpty()) {
            // если на карте нет точек 'P'
            throw new IllegalStateException("No monster spawn points defined on the map!");
            // return new Hex(0, 0);
        }
        return monsterSpawnPoints.stream()
                .filter(spawnPoint -> spawnPoint.getTemplateId().equals(templateId))
                .map(spawnPoint -> Hex.builder()
                        .q(spawnPoint.getPosition().getQ())
                        .r(spawnPoint.getPosition().getR())
                        .build())
                .toList();
    }
}
